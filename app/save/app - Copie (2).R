...
#############################################
  ...
  .box_num{
    ...
    font-weight: bold;
    padding: 0.75rem 1rem;
    # border-left: 1px solid #15354A;
    # border-right: 1px solid #15354A;
    # border-bottom: 1px solid #15354A;
                    ...
                    tabPanel("Dashboard",
                             div(class = "wrapper",
                               ...
                               div(class = "row_boxes",
                                     ...
                                     div(class = "box",
                                         div(class = "box_text", p("Comparisons with high GRADE")),
                                         div(class = "box_num", uiOutput('box_comp_high')),
                                     ),
                                     div(class = "box",
                                         div(class = "box_text", p("Comparisons with large efficacy")),
                               ...
                               div(class = "row_boxes_plot",
                                   div(class = "box_plot",
                                       ...
                                       div(class = "row_plots", ggiraphOutput("circle_interventions"))
                                   ),
                                   div(class = "box_plot",
...
server <- function(input, output, session) {
  ...
  # output$polar_outcomes <- renderPlot({
  #   data <- filterData() %>%
  #     group_by(Outcome) %>%
  #     summarise(value=n()) %>%
  #     mutate(id = 1:n())
  ...
  #     geom_text(data=label_data, aes(x=id, y=value+10, label=Outcome, hjust=hjust), color="black", fontface="bold",alpha=0.6, size=2.5, angle= label_data$angle, inherit.aes = FALSE )
  # })
  ...
  output$comp_eff = renderText({ paste0(nrow(filterData()  %>% filter(TE > 0.8))) })
  ...
  output$forest_imp = renderText({
    paste0("The number of comparisons is too large to be rendered in a figure (n=",
           nrow(filterData()),
           "), while your internet connexion allows a maximum of n=250 comparisons to be displayed.",
           " Please filter the dataset to visualize the results in a forest plot.")
  })
