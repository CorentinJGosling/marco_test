...
server <- function(input, output, session) {
  ...
  observeEvent(input$Age, {
    filtered_values_BD <- unique(dat$BD_stage[dat$Age %in% input$Age])
    filtered_values_DES <- unique(dat$design[dat$Age %in% input$Age])
    filtered_values_GRA <- unique(dat$Rank[dat$Age %in% input$Age])
    filtered_values_AMS <- unique(dat$AMSTAR[dat$Age %in% input$Age])
  ...
  observeEvent(input$BD_stage, {
    filtered_values_AGE <- unique(dat$Age[dat$BD_stage %in% input$BD_stage])
    filtered_values_DES <- unique(dat$design[dat$BD_stage %in% input$BD_stage])
    filtered_values_GRA <- unique(dat$Rank[dat$BD_stage %in% input$BD_stage])
    filtered_values_AMS <- unique(dat$AMSTAR[dat$BD_stage %in% input$BD_stage])
  ...
  observeEvent(input$design, {
    filtered_values_BD <- unique(dat$BD_stage[dat$Design %in% input$design])
    filtered_values_AGE <- unique(dat$Age[dat$Design %in% input$design])
    filtered_values_GRA <- unique(dat$Rank[dat$Design %in% input$design])
    filtered_values_AMS <- unique(dat$AMSTAR[dat$Design %in% input$design])
    updatePickerInput(session, 'BD_stage', choices = filtered_values_BD, selected = input$BD_stage)
    updatePickerInput(session, 'Age', choices = filtered_values_AGE, selected = input$Age)
    updatePickerInput(session, 'GRADE', choices = filtered_values_GRA, selected = input$GRADE)
    updatePickerInput(session, 'AMSTAR', choices = filtered_values_AMS, selected = input$AMSTAR)
  ...
  observeEvent(input$GRADE, {
    filtered_values_BD <- unique(dat$BD_stage[dat$GRADE %in% input$GRADE])
    filtered_values_DES <- unique(dat$design[dat$GRADE %in% input$GRADE])
    filtered_values_AGE <- unique(dat$Age[dat$GRADE %in% input$GRADE])
    filtered_values_AMS <- unique(dat$AMSTAR[dat$GRADE %in% input$GRADE])
    updatePickerInput(session, 'BD_stage', choices = filtered_values_BD, selected = input$BD_stage)
    updatePickerInput(session, 'design', choices = filtered_values_DES, selected = input$design)
    updatePickerInput(session, 'Age', choices = filtered_values_AGE, selected = input$Age)
    updatePickerInput(session, 'AMSTAR', choices = filtered_values_AMS, selected = input$AMSTAR)
  ...
  observeEvent(input$AMSTAR, {
    filtered_values_BD <- unique(dat$BD_stage[dat$AMSTAR %in% input$AMSTAR])
    filtered_values_DES <- unique(dat$design[dat$AMSTAR %in% input$AMSTAR])
    filtered_values_GRA <- unique(dat$Rank[dat$AMSTAR %in% input$AMSTAR])
    filtered_values_AGE <- unique(dat$Age[dat$AMSTAR %in% input$AMSTAR])
    updatePickerInput(session, 'BD_stage', choices = filtered_values_BD, selected = input$BD_stage)
    updatePickerInput(session, 'design', choices = filtered_values_DES, selected = input$design)
    updatePickerInput(session, 'GRADE', choices = filtered_values_GRA, selected = input$GRADE)
    updatePickerInput(session, 'Age', choices = filtered_values_AGE, selected = input$Age)
